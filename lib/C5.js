...
class Axis {
	...
	*/
	...
	axisGrid(args) {
		...
		this.g.debugInfo("style", {grid: style.grid, tick:style.tick, label: style.label})
	...
	strokePath(arr, args) {
		const ctx = this.g.ctx;
		const style = (args == undefined)? {} : args; 
		...
		if ("alpha" in style) {
			...
			this.g.debugInfo("alpha", style.alpha);
		}
		ctx.beginPath();
...
function float2Apply(f, zarr) {
	const N = Math.floor(zarr.length/2);
	const result = new Float64Array(N);
	for (let i = 0; i < N - 1; i += 1) {
		result[i] = f(zarr[2*i], zarr[2*i + 1]);
	}
	return result;
}
